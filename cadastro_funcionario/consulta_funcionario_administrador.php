<?php
  ...
  $result_funcionario = "SELECT * FROM funcionario LIMIT $inicio, $qnt_result_pg"; 
  ...
  <body>
    <div class="wrapper">
      ...
      <div id="content">
        <nav class="navbar navbar-expand-lg navbar-light menu" style="background-color:#E8E8E8">
          <div class="container-fluid">
            ...
            <h3 class="h3" style="margin-left: 140px">Consultar Funcionário</h3>
                  ...
                  <div class="col-sm-6 col-md-6">
                    <form class="form-inline" method="GET" action="pesquisar_administrador.php">
                      <div class="form-group">
                        <input type="text" name="pesquisar" class="form-control" id="exampleInputName2" placeholder="Nome do funcionário">&nbsp&nbsp
              ...
              <div class="row">
                ...
                <tbody>
                  <?php while($rows_funcionario = mysqli_fetch_assoc($resultado_funcionario)){ ?>
                    <tr>
                      <td>
                        <?php echo $rows_funcionario['cpfFuncionario']; ?>
                      </td>
                      <td>
                        <?php echo $rows_funcionario['nomeFuncionario']; ?>
                      </td>
                      <div class="col-md-3">
                        <td class="float-right">
                          <button type="button" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#myModal<?php echo $rows_funcionario['id']; ?>"><i class='fa fa-eye'></i></button>
                          <button type="button" class="btn btn-xs btn-warning" data-target="#exampleModal">
                            <?php echo "<a href='editar_cadastro_funcionario_administrador.php?id=" . $rows_funcionario['id']. "'><i class='fa fa-edit'></i></a>"?></button>
						  <button type="button" class="btn btn-xs btn-danger">
                            <?php echo "<a href='proc_apagar_funcionario.php?id=" . $rows_funcionario['id'] . "' data-confirm='Tem certeza que deseja excluir o item selecionado?'><i class='fa fa-trash-alt'></i></a>"?></button>						
						</td>
                    </tr>
                    </div>
              </div>
              <!--Inicio Modal -->
              <div class="modal fade" id="myModal<?php echo $rows_funcionario['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
                <div class="modal-dialog modal-lg" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                      <h4 class="modal-title text-left" id="myModalLabel"><?php echo $rows_funcionario['nomeFuncionario']; ?></h4> </div>
                    <div class="modal-body">
                      <div class="container-fluid">
                        <h4 class="modal-title text-left" id="myModalLabel">Cadastro de Funcionário</h4>
                        <br>
                        <p>
                          <div class="row">
                            <div class="col-md-6">
                              <?php echo'<strong>CPF: </strong>'.$rows_funcionario['cpfFuncionario']; ?></div>
                            <div class="col-md-6">
                              <?php echo'<strong>Data de Nascimento: </strong>'.$rows_funcionario['dataNascFuncionario']; ?></div>
                          </div>
                        </p>
                        <p>
                          <div class="row">
                            <div class="col-md-4">
                              <?php echo'<strong>Função: </strong>'.$rows_funcionario['funcaoFuncionario']; ?></div>
                          </div>
                        </p>
                        <p>
                          <div class="row">
                            <div class="col-md-6">
                              <?php echo'<strong>Telefone: </strong>'.$rows_funcionario['telResFuncionario']; ?></div>
                            <div class="col-md-6">
                              <?php echo'<strong>Celular: </strong>'.$rows_funcionario['celularFuncionario']; ?></div>
                          </div>
                        </p>
                        <p>
                          <div class="row">
                            <div class="col-md-6">
                              <?php echo'<strong>E-mail: </strong>'.$rows_funcionario['emailFuncionario']; ?></div>
                          </div>
                        </p>
                        <p>
                          <div class="row">
                            <div class="col-md-8">
                              <?php echo'<strong>Endereço: </strong>'.$rows_funcionario['enderecoFuncionario']; ?></div>
                          </div>
                        </p>
                        <p>
                          <div class="row">
                            <div class="col-md-6">
                              <?php echo'<strong>Bairro: </strong>'.$rows_funcionario['bairroFuncionario']; ?></div>
                            <div class="col-md-4">
                              <?php echo'<strong>Cidade: </strong>'.$rows_funcionario['cidadeFuncionario']; ?></div>
                          </div>
                        </p>
                        <p>
                          <div class="row">
                            <div class="col-md-6">
                              <?php echo'<strong>Estado: </strong>'.$rows_funcionario['estadoFuncionario']; ?>
                        </div>
                        <div class="col-md-4">
                          <?php echo'<strong>País: </strong>'.$rows_funcionario['paisFuncionario']; ?>
                        </div>
                        </div>
                        </p>
                        <p>
                          <div class="row">
                            <div class="col-md-4">
                              <?php echo'<strong>CEP: </strong>'.$rows_funcionario['cepFuncionario']; ?></p>
                        </div>
                        </div>
						<hr class="mb-4">
                          <div class="row">
                            <div class="col-4 mb-3"><a class="btn btn-primary" href="consulta_funcionario_administrador.php" role="button"><i class="fa fa-times"></i> Sair</a> </div>
                          </div>
                      </div>
                    </div>
                  </div>
      ...
      <?php  
        ...
        echo "<span class='page-link'><a href='consulta_funcionario_administrador.php?pagina=1'>Primeira</a></span>";
        echo '</li>';
  
        for($pag_ant = $pagina - $max_links; $pag_ant <= $pagina - 1; $pag_ant++){
		  if($pag_ant >= 1){
		    echo "<li class='page-item'><a class='page-link' href='consulta_funcionario_administrador.php?pagina=$pag_ant'>$pag_ant</a></li>";
		    }
		}
        ...
        for($pag_dep = $pagina + 1; $pag_dep <= $pagina + $max_links; $pag_dep++){
		  if($pag_dep <= $quantidade_pg){
		    echo "<li class='page-item'><a class='page-link' href='consulta_funcionario_administrador.php?pagina=$pag_dep'>$pag_dep</a></li>";
		    }
		}
        echo '<li class="page-item">';
        echo "<span class='page-link'><a href='consulta_funcionario_administrador.php?pagina=$quantidade_pg'>Última</a></span>";
        echo '</li>';
        echo '</ul>';
        echo '</nav>';
