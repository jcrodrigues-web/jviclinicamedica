  ...
  <body>
    <div class="wrapper">
      ...
      <div id="content">
        <nav class="navbar navbar-expand-lg navbar-light menu" style="background-color:#E8E8E8">
          <div class="container-fluid">
            ...
            <h3 class="h3" style="margin-left: 150px">Consultar Usuário</h3>
                  ...
                  <div class="col-sm-6 col-md-6">
                    <form class="form-inline" method="GET" action="pesquisar_administrador.php">
                      <div class="form-group">
                        <input type="text" name="pesquisar" class="form-control" id="exampleInputName2" placeholder="Nome do Usuário">&nbsp&nbsp
                  ...
                  <table class="table table-striped">
                    ...
                    <tbody>
                      <?php while($rows_usuario = mysqli_fetch_assoc($resultado_usuario)){ ?>
                        <tr>
                          <td>
                            <?php echo $rows_usuario['id']; ?>
                          </td>
                          <td>
                            <?php echo $rows_usuario['nome']; ?>
                          </td>
                          <div class="col-md-3">
                            <td class="float-right">
                              <button type="button" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#myModal<?php echo $rows_usuario['id']; ?>"><i class='fa fa-eye'></i></button>
                              <button type="button" class="btn btn-xs btn-warning" data-target="#exampleModal">
                                <?php echo "<a href='editar_usuario_administrador.php?id=" . $rows_usuario['id']. "'><i class='fa fa-edit'></i></a>"?></button>
							  <button type="button" class="btn btn-xs btn-danger">
								<?php echo "<a href='proc_apagar_usuario.php?id=" . $rows_usuario['id'] . "' data-confirm='Tem certeza que deseja excluir o item selecionado?'><i class='fa fa-trash-alt'></i></a>"?></button>		
							</td>
                          </div>
                        </tr>
                        <!--Inicio Modal -->
                        <div class="modal fade" id="myModal<?php echo $rows_usuario['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
                          <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                              <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                <h4 class="modal-title text-left" id="myModalLabel"><?php echo $rows_usuario['nome']; ?></h4> </div>
                              <div class="modal-body">
                                <div class="container-fluid">
                                  <h4 class="modal-title text-left" id="myModalLabel">Cadastro de Usuário</h4>
                                  <br>
                                  <p>
                                    <div class="row">
                                      <div class="col-md-6">
                                        <?php echo "<strong>E-mail: </strong>".$rows_usuario['email']; ?></div>
                                    </div>
                                  </p>
                                  <p>
                                    <div class="row">
                                      <div class="col-md-6">
                                        <?php echo'<strong>Login: </strong>'.$rows_usuario['login']; ?></div>
                                    </div>
                                  </p>
                                  <p>
                                    <div class="row">
                                      <div class="col-md-4">
                                        <?php echo'<strong>Nível de Acesso: </strong>'.$rows_usuario['nivel_acesso_id']; ?></div>
                                    </div>
                                  </p>
								  <hr class="mb-4">
									<div class="row">
										<div class="col-4 mb-3"><a class="btn btn-primary" href="consulta_usuario_administrador.php" role="button"><i class="fa fa-times"></i> Sair</a> </div>
									</div>
                                </div>
                              </div>
                            </div>
      ...
      <?php  
        ...
        echo "<span class='page-link'><a href='consulta_usuario_administrador.php?pagina=1'>Primeira</a></span>";
        echo '</li>';
  
        for($pag_ant = $pagina - $max_links; $pag_ant <= $pagina - 1; $pag_ant++){
		  if($pag_ant >= 1){
		    echo "<li class='page-item'><a class='page-link' href='consulta_usuario_administrador.php?pagina=$pag_ant'>$pag_ant</a></li>";
		    }
		}
        ...
        for($pag_dep = $pagina + 1; $pag_dep <= $pagina + $max_links; $pag_dep++){
		  if($pag_dep <= $quantidade_pg){
		    echo "<li class='page-item'><a class='page-link' href='consulta_usuario_administrador.php?pagina=$pag_dep'>$pag_dep</a></li>";
		    }
		}
        echo '<li class="page-item">';
        echo "<span class='page-link'><a href='consulta_usuario_administrador.php?pagina=$quantidade_pg'>Última</a></span>";
        echo '</li>';
        echo '</ul>';
        echo '</nav>';
