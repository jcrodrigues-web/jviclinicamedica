...
//Selecionar todos os historicos da tabela
$result_historico = "SELECT * FROM historico WHERE nomeHistPaciente LIKE '%$valor_pesquisar%'";
$resultado_historico = mysqli_query($conexao, $result_historico);

//Selecionar os historicos a serem apresentado na página **** '%$valor_pesquisar%' limit $incio, $quantidade_pg ****
$result_historicos = "SELECT * FROM historico WHERE nomeHistPaciente LIKE '%$valor_pesquisar%'";
$resultado_historicos = mysqli_query($conexao, $result_historicos);
$total_historicos = mysqli_num_rows($resultado_historicos);
?>
      ...
      <div id="content">
        <nav class="navbar navbar-expand-lg navbar-light menu" style="background-color:#E8E8E8">
          <div class="container-fluid">
            ...
            <h3 class="h3" style="margin-left: 100px">Consultar Histórico do Paciente</h3>
          ...
          <div class="container">
            <div class="container theme-showcase" role="main">
                  ...
                  <div class="col-sm-6 col-md-6">
                    <form class="form-inline" method="GET" action="pesquisar_administrador.php">
                      <div class="form-group">
						...
						<a class="btn btn-primary" href="consulta_historico_administrador.php" role="button">Voltar</a>
                      </div>
                    </form>
                  </div>
                </div>
              ...
              <div class="row">
                ...
                <tbody>
                  <?php while($rows_historicos = mysqli_fetch_assoc($resultado_historicos)){ ?>
                    <tr>
                      <td>
                        <?php echo $rows_historicos['id']; ?>
                      </td>
                      <td>
                        <?php echo $rows_historicos['nomeHistPaciente']; ?>
                      </td>
                      <div class="col-md-3">
                        <td class="float-right">
                          <button type="button" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#myModal<?php echo $rows_historicos['id']; ?>"><i class='fa fa-eye'></i></button>
                          <button type="button" class="btn btn-xs btn-warning" data-target="#exampleModal">
                            <?php echo "<a href='editar_historico_administrador.php?id=" . $rows_historicos['id']. "'><i class='fa fa-edit'></i></a>"?></button>
						  <button type="button" class="btn btn-xs btn-danger">
                            <?php echo "<a href='proc_apagar_historico_administrador.php?id=" . $rows_historicos['id'] . "' data-confirm='Tem certeza que deseja excluir o item selecionado?'><i class='fa fa-trash-alt'></i></a>"?></button>
						</td>
                      </div>
                    </tr>
              </div>
              ...
              <!--Inicio Modal -->
              <div class="modal fade" id="myModal<?php echo $rows_historicos['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
                <div class="modal-dialog modal-lg" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                      <h4 class="modal-title text-left" id="myModalLabel"><?php echo $rows_historicos['nomeHistPaciente']; ?></h4> </div>
                    <div class="modal-body">
                      <div class="container-fluid">
                        <h4 class="modal-title text-left" id="myModalLabel">Histórico do Paciente</h4>
                        <br>
                        <p>
                          <div class="row">
                            <div class="col-md-6">
                              <?php echo'<strong>Convênio: </strong>'.$rows_historicos['convenioHistPaciente']; ?></div>
                          </div>
                        </p>
                        <p>
                          <div class="row">
                            <div class="col-md-6">
                              <?php echo'<strong>Médico Solicitante: </strong>'.$rows_historicos['medicoHistPaciente']; ?></div>
                          </div>
                        </p>
                        <p>
                          <div class="row">
                            <div class="col-md-6">
                              <?php echo'<strong>Data da Solicitação: </strong>'.$rows_historicos['dataHistPaciente']; ?></div>
                          </div>
                        </p>
                        <p>
                          <div class="row">
                            <div class="col-md-6">
                              <?php echo'<strong>Tipo de Exame: </strong>'.$rows_historicos['exameHistPaciente']; ?></div>
                          </div>
                        </p>
                        <p>
                          <div class="row">
                            <div class="col-md-6">
                              <?php echo'<strong>Medicamento Prescrito: </strong>'.$rows_historicos['medicamentoHistPaciente']; ?></div>
                          </div>
                        </p>
                        <p>
                          <div class="row">
                            <div class="col-md-8">
                              <?php echo'<strong>Resultados Obtidos: </strong>'.$rows_historicos['resultadoHistPaciente']; ?></div>
                          </div>
                        </p>
						<hr class="mb-4">
                          <div class="row">
                            <div class="col-4 mb-3"><a class="btn btn-primary" href="consulta_historico_administrador.php" role="button"><i class="fa fa-times"></i> Sair</a> </div>
                          </div>
                      </div>
                    </div>
                  </div>
