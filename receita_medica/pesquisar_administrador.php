...
//Selecionar todos os receitas da tabela
$result_receita = "SELECT * FROM receita WHERE nomePacienteReceita LIKE '%$valor_pesquisar%'";
$resultado_receita = mysqli_query($conexao, $result_receita);

//Selecionar os receitas a serem apresentado na página **** '%$valor_pesquisar%' limit $incio, $quantidade_pg ****
$result_receitas = "SELECT * FROM receita WHERE nomePacienteReceita LIKE '%$valor_pesquisar%'";
$resultado_receitas = mysqli_query($conexao, $result_receitas);
$total_receitas = mysqli_num_rows($resultado_receitas);
?>
          ...
          <div class="screen container">
            <div class="container theme-showcase" role="main">
                  ...
                  <div class="col-sm-6 col-md-6">
                    <form class="form-inline" method="GET" action="pesquisar_administrador.php">
                      <div class="form-group">
                        <input type="text" name="pesquisar" class="form-control" id="exampleInputName2" placeholder="Nome do Paciente">&nbsp
                        <button type="submit" class="btn btn-primary"><i class="fa fa-search"></i> Pesquisar</button>&nbsp&nbsp
						<a class="btn btn-primary" href="consulta_receita_administrador.php" role="button">Voltar</a>
                      </div>
                    </form>
                  </div>
                </div>
              ...
              <div class="row">
                ...
                <tbody>
                  <?php while($rows_receitas = mysqli_fetch_assoc($resultado_receitas)){ ?>
                    <tr>
                      <td>
                        <?php echo $rows_receitas['cpfPaciente']; ?>
                      </td>
                      <td>
                        <?php echo $rows_receitas['nomePacienteReceita']; ?>
                      </td>
                      <div class="col-md-3">
                        <td class="float-right">
                          <button type="button" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#myModal<?php echo $rows_receitas['id']; ?>"><i class='fa fa-eye'></i></button>
                          <button type="button" class="btn btn-xs btn-warning" data-target="#exampleModal">
                            <?php echo "<a href='editar_receita_administrador.php?id=" . $rows_receitas['id']. "'><i class='fa fa-edit'></i></a>"?></button>
                          <button type="button" class="btn btn-xs btn-danger">
                            <?php echo "<a href='proc_apagar_receita_administrador.php?id=" . $rows_receitas['id'] . "' data-confirm='Tem certeza que deseja excluir o item selecionado?'><i class='fa fa-trash-alt'></i></a>"?></button>
                        </td>
                      </div>
                    </tr>
              </div>
              <!--Inicio Modal -->
                <div class="printable modal fade" id="myModal<?php echo $rows_receitas['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel">
                  <div class="modal-lg" role="document">
                    <div class="modal-content">
                      <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title text-left" id="myModalLabel"><?php echo $rows_receitas['nomePacienteReceita']; ?></h4> </div>
                      <div class="modal-body">
                        <div class="container-fluid">
                          <div class="row">
                            <div class="col-md-7">
                              <?php echo'<strong>Idade: </strong>'.$rows_receitas['idadePacienteReceita']; ?></div>
                          </div>
                          <div class="row">
                            <div class="col-md-8">
                              <?php echo'<strong>Data de Nascimento: </strong>'.$rows_receitas['dataNascPacienteReceita']; ?></div>
                          </div>
                          <div class="row">
                            <div class="col-md-7">
                              <?php echo'<strong>Convênio: </strong>'.$rows_receitas['convenioPacienteReceita']; ?></div>
                            <div class="col-md-5">
                              <?php echo'<strong>Data Solicitação: </strong>'.$rows_receitas['dataPacienteReceita']; ?></div>
                          </div>
                          <div class="row">
                            <div class="col-md-7">
                              <?php echo'<strong>Nome do Médico: </strong>'.$rows_receitas['medicoPacienteReceita']; ?></div>
                            <div class="col-md-5">
                              <?php echo'<strong>CRM: </strong>'.$rows_receitas['crmPacienteReceita']; ?></div>
                          </div>
                          <hr class="mb-4">
                          ...
                          <div class="row">
                            <div class="col-md-12">
                              <p>
                                <?php echo $rows_receitas['receituarioPacienteReceita']; ?>
                            </div>
                          </div>
                          <hr class="mb-4">
                          <div class="screen row">
							...
							<a class="btn btn-primary" href="consulta_receita_administrador.php" role="button"><i class="fa fa-times"></i> Sair</a>
							</div>
                          </div>
                        </div>
                      </div>
